...
| k | **`text_parts` raises `ValueError` for candidate text outside the grammar, and the catch-all reports it as `MALFORMED_RECORD`.** The surrounding code's intent is plainly `DISPLAY_VALUE` — it calls `text_parts(...) != ek` and raises that code — but the parser raises instead of returning, so the comparison never happens. | The candidate's defect is real and the verdict is right, but it is reported under the wrong code with an exception string as the detail, and every later check on that record is abandoned. Same catch-all, same abandonment as F3/D1. | Owner |
| l | **A child's `PARENT_UNRESOLVED` carries the evidence of whichever parent problem is found first.** Where the parent has both `MISSING_REQUIRED` (evidence `[]`) and `SOURCE_CONFLICT` (evidence `[a, b]`), the first wins the field and the child's issue is emitted with empty evidence. The candidate that cites the conflicting sources is then failed with `ISSUE_EVIDENCE`. | Bears directly on the 2026-09-20 ruling that "conflicting sources are reported to the seller showing where the conflict is". A blocking issue with no evidence does not show where. | Owner |
