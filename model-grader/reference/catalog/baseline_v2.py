...
def check_setup(case):
    if case.get('contract_version') not in ('1.4','1.5'): raise SetupError('contract version')
...
def grade(case,candidate):
    ...
    for key in candidate.keys()-{'case_id','records','catalog'}: err('UNSUPPORTED_OUTPUT_FIELD',field=key)
    ...
    for sku,r in records.items():
        ...
        try:
            ...
            for key in o.keys()-{'sku','role','parent_sku','fields','evidence','status','issues','display','measurements','record_status'}:
    ...
    return {'verdict':'FAIL' if errors else 'PASS','errors':errors,'warnings':warnings,'records':per,'guided_help':guided,'correctly_handled':sum(x['handling']=='PASS' for x in per) if not global_error else 0,'publishable':sum(x['handling']=='PASS' and x['expected_status']=='READY' for x in per) if not global_error else 0,'incomplete':sum(x['handling']=='PASS' and x['expected_status']=='BLOCKED' for x in per)}
