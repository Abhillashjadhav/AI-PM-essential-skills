...
5. Resolve conflicting information against a supplier-designated authoritative source, identified and located in the input registry. Do not choose a source based on model confidence, order, or an invented authority ranking. Unresolved conflicts on required fields remain blocked with a guided request for the missing authority or supplier correction. **Unresolved conflicts on an optional field do not block the record** (owner decision 4): withhold the disputed field, publish the otherwise valid product without it, and emit supplier guidance naming the field and the conflicting sources. Equal duplicate sources are not conflicts.
